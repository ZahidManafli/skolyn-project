...
import { useLocalization } from "../../assests/lang";
...
export default function AuthCard() {
  ...
  const handleLoading = async (e: React.FormEvent): Promise<void> => {
    e.preventDefault();
    if (isloading) return; // 2 dəfə klikləməyə qarşı qoruma
    ...
    try {
      const response = await axios.post(
          `http://167.86.97.169/zahren/api/login/`,
          loginData
      );
  ...
  const validateField = (name: string, value: string, login: boolean): string => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    const login = (e.target as HTMLFormElement).closest(".login") !== null;
    ...
    try {
      const response = await axios.post<LoginResponse>(
          `http://167.86.97.169:8000/zahren/api/${url}/`,
          dataObj
      );
